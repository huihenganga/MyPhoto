...
public class AttentionServiceImpl implements AttentionService {
	...
	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#getCollectCidAndUidAndType(java.lang.String, java.lang.String, java.lang.Integer)
	 */
	@Override
	public Collection getCollectCidAndUidAndType(Integer beCollecId, String userId) {
		return collectionDao.findFirstByHql(
				"from Collection where collectionId=? and collectBy=?",beCollecId,userId);
	}

	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#saveOrUpdateCollection(com.myphoto.entity.Collection)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#getAttentionList(com.myphoto.entity.base.PageObject, java.lang.Integer)
	 */
	@Override
	public PageObject getAttentionProductList(PageObject pageObject, String userId) {
		List<Attention> list = attentionDao.findByHql(" from Attention v where 1=1 and v.status=1 and" +
...
//				}
		...
		if(!str.equals("()")&&!str.equals("")){
			StringBuffer hql = new StringBuffer("from Product where 1=1 and shareId in"+str);
			...
			if(list3.size()>0){
				for(int i=0;i<list3.size();i++){
					Map<String, Object> map = new HashMap<String, Object>();
					User u = userDao.findFirstByHql(
							"from User where id=?", list3.get(i).getShareId());
					Attention attention =attentionDao.findFirstByHql(" from Attention " +
							"where attentionBy=? and attentionTo=?", userId,list3.get(i).getShareId());
					Collection collection = collectionDao.findFirstByHql(" from Collection " +
							"where collectBy=? and collectionId=?", userId,list3.get(i).getId());
					ProductCount pc = productCountDao.findFirstByHql(" from ProductCount " +
							"where productId=?", list3.get(i).getId());
					map.put("id", list3.get(i).getId());
					map.put("title", list3.get(i).getTitle());
					map.put("coveUrl", list3.get(i).getCoveUrl());
					map.put("publishTime", list3.get(i).getPublishTime().getTime());
	...
	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#getCollectArtShow(java.lang.Integer, java.lang.String)
	 */
	@Override
	public RUserArtShow getCollectArtShow(Integer artShowId, String userId) {
		return rUserArtShowDao.findFirstByHql(
				"from RUserArtShow where artShowId=? and userId=?",artShowId,userId);
	}

	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#saveOrUpdateRUserArtShow(com.myphoto.entity.RUserArtShow)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#getRecommendProductList(com.myphoto.entity.base.PageObject, java.lang.Integer)
	 */
	@Override
	public PageObject getRecommendProductList(PageObject pageObject,
			String userId) {
		StringBuffer hql = new StringBuffer("from Product order by publishTime desc");
		PageObject pg = productDao.findForPageByHql(pageObject, hql.toString());
		List<Product> dateCount = productDao.findByHql(hql.toString());
		...
		if(list.size()>0){
			for(int i=0;i<list.size();i++){
				Map<String, Object> map = new HashMap<String, Object>();
				User u = userDao.findFirstByHql(
						"from User where id=?", list.get(i).getShareId());
				Attention attention =attentionDao.findFirstByHql(" from Attention " +
						"where attentionBy=? and attentionTo=?", userId,list.get(i).getShareId());
				Collection collection = collectionDao.findFirstByHql(" from Collection " +
						"where collectBy=? and collectionId=?", userId,list.get(i).getId());
				ProductCount pc = productCountDao.findFirstByHql(" from ProductCount " +
	...
	/* (non-Javadoc)
	 * @see com.myphoto.service.AttentionService#getCollectProductList(com.myphoto.entity.base.PageObject, java.lang.String)
	 */
	@Override
	public PageObject getCollectProductList(PageObject pageObject, String userId) {
		List<Collection> list = collectionDao.findByHql(" from Collection v where 1=1 and v.status=1 and" +
				" v.collectBy=?",userId);
		String str = "";
		if(list.size()>0){
			str+="(";
...
//					str=str+list.get(i).getAttentionTo()+",";
//				}
				if(null!=(list.get(i).getCollectionId())&&0!=(list.get(i).getCollectionId())){
					str=str+list.get(i).getCollectionId()+",";
				}
			}
			if(null!=list.get(list.size()-1).getCollectionId()&&0!=list.get(list.size()-1).getCollectionId()){
		...
		if(!str.equals("()")&&!str.equals("")){
			StringBuffer hql = new StringBuffer("from Product where 1=1 and id in"+str);
			hql.append(" order by publishTime desc");
			...
			if(list3.size()>0){
				for(int i=0;i<list3.size();i++){
					Map<String, Object> map = new HashMap<String, Object>();
					User pc = userDao.findFirstByHql(
							"from User where id=?", list3.get(i).getShareId());
					Attention attention =attentionDao.findFirstByHql(" from Attention " +
							"where attentionBy=? and attentionTo=?", userId,list3.get(i).getShareId());
					Collection collection = collectionDao.findFirstByHql(" from Collection " +
	...
	public PageObject getCollectArtShowList(PageObject pageObject, String userId) {
		List<RUserArtShow> list = rUserArtShowDao.findByHql(" from RUserArtShow v where 1=1 and v.status=1 and" +
				" v.userId=?",userId);
		String str = "";
		if(list.size()>0){
			str+="(";
...
//					str=str+list.get(i).getAttentionTo()+",";
//				}
				if(null!=(list.get(i).getArtShowId())&&0!=(list.get(i).getArtShowId())){
					str=str+list.get(i).getArtShowId()+",";
				}
			}
			if(null!=list.get(list.size()-1).getArtShowId()&&0!=list.get(list.size()-1).getArtShowId()){
		...
		if(!str.equals("()")&&!str.equals("")){
			StringBuffer hql = new StringBuffer("from ArtShow where 1=1 and id in"+str);
			...
			if(list3.size()>0){
				for(int i=0;i<list3.size();i++){
					Map<String, Object> map = new HashMap<String, Object>();
					RUserArtShow rUserArtShow = rUserArtShowDao.findFirstByHql(" from RUserArtShow " +
							"where userId=? and artShowId=?", userId,list3.get(i).getId());
					Long count = productArtShowDao.findCountBySql(
							"select count(*) from ProductArtShow where displayId=? ",list3.get(i).getId() );
					map.put("id", list3.get(i).getId());
					map.put("title", list3.get(i).getTitle());					
					if(count>0){
					...
					if(null!=rUserArtShow&&rUserArtShow.getStatus()==1){
						map.put("collectionTIme",rUserArtShow.getCollectTime().getTime());
						map.put("collectionStatus",1);
					}else{
	...
	public PageObject getProductListBuUserId(PageObject pageObject,
			String userId) {
		StringBuffer hql = new StringBuffer("from Product where shareId='"+userId);
		hql.append("' order by publishTime desc");
		PageObject pg = productDao.findForPageByHql(pageObject, hql.toString());
		List<Product> dateCount = productDao.findByHql(hql.toString());
		List<Product> list = pageObject.getData();
		...
		if(list.size()>0){
			for(int i=0;i<list.size();i++){
				...
				ProductCount pc = productCountDao.findFirstByHql(" from ProductCount " +
						"where productId=?", list.get(i).getId());
				ProductArtShow productArtShow =productArtShowDao.findFirstByHql(" from ProductArtShow " +
						"where productId=?",list.get(i).getId());
				map.put("artShowTitle", "未参展");
				if(null!=productArtShow){
					ArtShow artShow =artShowDao.findFirstByHql(" from ArtShow " +
							"where id=?",productArtShow.getDisplayId());
					if(null!=artShow){
...
//				map.put("background", list.get(i).getBackground());
	...
	public PageObject getHotProductList(PageObject pageObject) {
		...
		PageObject pg = vProdCollectCountDao.findForPageByHql(pageObject, hql.toString());
		return pg;
	}
	
	@Override
	public PageObject getArtShowList(PageObject pageObject) {
		StringBuffer hql = new StringBuffer("from ArtShow order by createTime desc");
		PageObject pg = artShowDao.findForPageByHql(pageObject, hql.toString());
		return pg;
	}
